...
#if DYN_ANEMO_PRESS
    	if(motorMode == 1) //mode 1 check motor threshold pour alume le moteur
    	{
    		if(anem_speed > motor_threshold){
    		    dxl_torque(1, TORQUE_ON);
    			dxl_LED(1, LED_ON);
    		    dxl_setGoalVelocity(1 ,motor_speed);
    		} else {        //
    			dxl_LED(1, LED_OFF);
    			dxl_setGoalVelocity(1 , 0);
    		}
    	}
    	else if(motorMode == 2) //la vitesse de anem c la meme du moteur, depend pas sur le moteur threshold
    	{
    			dxl_torque(1, TORQUE_ON);
    			dxl_LED(1, LED_OFF);
    			dxl_setGoalVelocity(1 ,10*(anem_speed));
    	}
    	else  //si le mode different de 1 ou 2 le moteur nallume pas
    		{
    			dxl_torque(1, TORQUE_OFF);
    	    	dxl_LED(1, LED_OFF);
    	    	dxl_setGoalVelocity(1 , 0);
...
void can_callback(void) //quand on recoit des instruction sur le receive
{
	...
	if (msg_rcv.id == 0x04){
	 motorMode = msg_rcv.data[0]; //motor_mode 0-DESACTIVE 1-ACTIVE (NORMAL MODE) 2- MOTOR SPEED = ANEM
	 motor_speed = msg_rcv.data[1];
	 motor_threshold = msg_rcv.data[2];
	 }
